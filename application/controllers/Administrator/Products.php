...
class Products extends CI_Controller
{
    ...
    public function addProduct()
    {
        ...
        try {
            ...
            unset($product['Product_Purchase_Rate']);
            unset($product['Product_SellingPrice']);
            unset($product['Product_WholesaleRate']);
            $product['Product_Purchase_Rate'] = $this->mt->convert($productObj->Product_Purchase_Rate);
            $product['Product_SellingPrice'] = $this->mt->convert($productObj->Product_SellingPrice);
            $product['Product_WholesaleRate'] = $this->mt->convert($productObj->Product_WholesaleRate);
        ...
        $stock = $this->db->query("
            select
                ...
                (select ifnull(sum(srd.SaleReturnDetails_ReturnQuantity), 0)
                    ...
                    where srd.Status = 'a'
                    and srd.SaleReturnDetailsProduct_SlNo = p.Product_SlNo
