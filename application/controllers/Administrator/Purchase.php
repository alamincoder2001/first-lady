...
class Purchase extends CI_Controller
{
    ...
    public function getPurchases()
    {
        ...
        if (isset($data->purchaseId) && $data->purchaseId != null) {
            ...
            $purchaseReturn = $this->db->query("SELECT prd.*,p.Product_Name,p.Product_Code
            ...
            $res['purchaseReturn'] = $purchaseReturn;
        }
    ...
    public function addPurchase()
    {
        ...
        try {
            ...
            $purchase = array(
                ...
                'PurchaseMaster_BillNo'         => $data->purchase->Bill_no,
                ...
                'PurchaseMaster_ReturnAmount'   => $data->purchase->returnTotal,
            ...
            if (count($data->returnCart) > 0) {
                $purchaseReturn = array(
                    'PurchaseMaster_InvoiceNo'    => $invoice,
                    'Supplier_IDdNo'              => $supplierId,
                    'PurchaseReturn_ReturnDate'   => $data->purchase->purchaseDate,
                    'PurchaseReturn_ReturnAmount' => $data->purchase->returnTotal,
                    'PurchaseReturn_Description'  => $data->purchase->returnNote,
                    'Status'                      => 'a',
                    'AddBy'                       => $this->session->userdata("FullName"),
                    'AddTime'                     => date('Y-m-d H:i:s'),
                    'PurchaseReturn_brunchID'     => $this->session->userdata('BRANCHid')
                ...
                $this->db->insert('tbl_purchasereturn', $purchaseReturn);
                $returnInsertId = $this->db->insert_id();

                foreach ($data->returnCart as $rProduct) {
                    $returnDetails = array(
                        'PurchaseReturn_SlNo'                  => $returnInsertId,
                        'PurchaseReturnDetailsProduct_SlNo'    => $rProduct->productId,
                        'PurchaseReturnDetails_ReturnQuantity' => $rProduct->quantity,
                        'PurchaseReturnDetails_ReturnAmount'   => $rProduct->purchaseRate,
                        'Status'                               => 'a',
                        'AddBy'                                => $this->session->userdata("FullName"),
                        'AddTime'                              => date('Y-m-d H:i:s'),
                        'PurchaseReturnDetails_brachid'        => $this->session->userdata('BRANCHid')
                    ...
                    $inventoryCount = $this->db->query("select * from tbl_currentinventory where product_id = ? and branch_id = ?", [$rProduct->productId, $this->session->userdata('BRANCHid')])->num_rows();

                    $this->db->query("UPDATE tbl_currentinventory 
                    set purchase_return_quantity = purchase_return_quantity + ? 
                    where product_id = ?
                    and branch_id = ?
                ", [$rProduct->quantity, $rProduct->productId, $this->session->userdata('BRANCHid')]);
                }
            }
    ...
    public function updatePurchase()
    {
        ...
        try {
            ...
            // return purchase remove start
            $dataReturn = $this->db->query("SELECT `PurchaseReturn_SlNo` FROM `tbl_purchasereturn` WHERE `PurchaseMaster_InvoiceNo` = ? and Status = 'a'", $data->purchase->invoice)->row();

            if ($dataReturn) {

                $oldPurchaseReturnDetails = $this->db->query("SELECT prd.*
                    FROM tbl_purchasereturn pr
                    LEFT JOIN tbl_purchasereturndetails prd on prd.PurchaseReturn_SlNo = pr.PurchaseReturn_SlNo
                    WHERE pr.PurchaseMaster_InvoiceNo = ?
                    and pr.Status = 'a'
                    and prd.Status = 'a'
                ", $data->purchase->invoice)->result();


                $this->db->set('Status', 'd')->where('PurchaseMaster_InvoiceNo', $data->purchase->invoice)->update("tbl_purchasereturn");
                $this->db->set('Status', 'd')->where('PurchaseReturn_SlNo', $dataReturn->PurchaseReturn_SlNo)->update("tbl_purchasereturndetails");

                foreach ($oldPurchaseReturnDetails as $rProduct) {
                    $previousStock = $this->mt->productStock($rProduct->PurchaseReturnDetailsProduct_SlNo);
                    ...
                    where product_id = ?
                    and branch_id = ?
                ", [$rProduct->PurchaseReturnDetails_ReturnQuantity, $rProduct->PurchaseReturnDetailsProduct_SlNo, $this->session->userdata('BRANCHid')]);
                }
            }
            // return purchase remove end
