...
class Page extends CI_Controller {
    ...
    public function image_upload($file_name_get){
		...
		move_uploaded_file($file_temp,'uploads/Branch/'.$new_name);
		return $new_name;
	}
    ...
    public function addBranch()  {
        ...
        try{
            ...
            $newBranch = array(
                ...
                'branch_logo' => $this->image_upload($_FILES['branch_logo']) ?? '',
                'Brunch_sales' => '2',
    ...
    public function updateBranch()  {
        ...
        try{
            ...
            $image = '';
            $oldImage = $this->db->query("SELECT * FROM tbl_brunch WHERE brunch_id = ?
            ", $branchId)->row();
            if(isset($_FILES['branch_logo'])){
                $image = $this->image_upload($_FILES['branch_logo']);
                if($oldImage->branch_logo != null)
                {
                    $img_unlink = 'uploads/Branch/'.$oldImage->branch_logo;
                    unlink($img_unlink);
                }
            } else {
                $image = $oldImage->branch_logo;
            }
