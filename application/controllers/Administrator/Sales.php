...
class Sales extends CI_Controller
{
    ...
    public function updateSales()
    {
        ...
        try {
            ...
            // Sale Return Part Remove Start
                $dataReturn = $this->db->query("SELECT `SaleReturn_SlNo` FROM `tbl_salereturn` WHERE `SaleMaster_InvoiceNo` = ? and Status = 'a'", $data->sales->invoiceNo)->row();
                    if ($dataReturn) {

                        $oldSalesReturnDetails = $this->db->query("SELECT srd.*
                            FROM tbl_salereturn sr
                            LEFT JOIN tbl_salereturndetails srd on srd.SaleReturn_IdNo = sr.SaleReturn_SlNo
                            WHERE sr.SaleMaster_InvoiceNo = ?
                            and sr.Status = 'a'
                            and srd.Status = 'a'
                        ", $data->sales->invoiceNo)->result();
        
                        $this->db->set('Status', 'd')->where('SaleMaster_InvoiceNo', $data->sales->invoiceNo)->update("tbl_salereturn");
                        $this->db->set('Status', 'd')->where('SaleReturn_IdNo', $dataReturn->SaleReturn_SlNo)->update("tbl_salereturndetails");
        
                        foreach ($oldSalesReturnDetails as $rProduct) {
            ...
            // Sale Return Part Remove End

            // SALE RETURN PART START
                 // return part start
            if (count($data->returnCart) > 0) {
                $salesReturn = array(
                    'SaleMaster_InvoiceNo'    => $data->sales->invoiceNo,
                ...
                foreach ($data->returnCart as $rProduct) {
                    $returnDetails = array(
                        'SaleReturn_IdNo'                  => $returnInsertId,
                        'SaleReturnDetailsProduct_SlNo '   => $rProduct->productId,
                        'SaleReturnDetails_ReturnQuantity' => $rProduct->quantity,
                        'SaleReturnDetails_ReturnAmount'   => $rProduct->salesRate,
                        'Status'                           => 'a',
                        'AddBy'                            => $this->session->userdata("FullName"),
                        'AddTime'                          => date('Y-m-d H:i:s'),
                        'SaleReturnDetails_brunchID'       => $this->session->userdata('BRANCHid')
                    ...
                    if ($inventoryCount == 0) {
                        $inventory = array(
                            ...
                            'sales_return_quantity' => $rProduct->quantity,
                            'branch_id'             => $this->session->userdata('BRANCHid')
                        );
