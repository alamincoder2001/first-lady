...
class Supplier extends CI_Controller
{
    ...
    public function addSupplier()
    {
        ...
        try {
            ...
            unset($supplier['previous_due']);
            $supplier['previous_due'] = $this->mt->convert($supplierObj->previous_due);
    ...
    public function getSuppliers()
    {
        $suppliers = $this->db->query("
            select 
            *,
            concat(Supplier_Code, ' - ', Supplier_Name, ' - ', Supplier_Mobile) as display_name
            from tbl_supplier
            where Status = 'a'
            and Supplier_Type != 'G'
    ...
    public function getSupplierLedger()
    {
        ...
        $payments = $this->db->query("
            select
                ...
                pm.PurchaseMaster_BillNo as bill_no,
                (select ifnull(sum(pd.PurchaseDetails_TotalQuantity),0) from tbl_purchasedetails pd where pd.PurchaseMaster_IDNo = pm.PurchaseMaster_SlNo) as purchase_qty,
                (select ifnull(sum(prd.PurchaseReturnDetails_ReturnQuantity),0) from tbl_purchasereturn pr
                left join tbl_purchasereturndetails prd on prd.PurchaseReturn_SlNo = pr.PurchaseReturn_SlNo
                where pr.PurchaseMaster_InvoiceNo = pm.PurchaseMaster_InvoiceNo and pr.Status = 'a') as p_return_qty,
                pm.PurchaseMaster_TotalAmount as bill,
                pm.PurchaseMaster_PaidAmount as paid,
                pm.PurchaseMaster_DiscountAmount as discount,
                (pm.PurchaseMaster_TotalAmount - pm.PurchaseMaster_PaidAmount) as due,
                0.00 as returned,
                0.00 as cash_received,
                0.00 as balance,
                pm.AddTime as datetime
            ...
            select
                ...
                '' as bill_no,
                0.00 as purchase_qty,
                0.00 as p_return_qty,
                0.00 as bill,
                sp.SPayment_amount as paid,
                sp.SPayment_discount as discount,
                0.00 as due,
                0.00 as returned,
                0.00 as cash_received,
                0.00 as balance,
                sp.SPayment_AddDAte as datetime
            ...
            select 
                ...
                0.00 as bill,
                0.00 as paid,
                sp2.SPayment_discount as discount,
                0.00 as due,
                0.00 as returned,
                sp2.SPayment_amount as cash_received,
                0.00 as balance,
                sp2.SPayment_AddDAte as datetime
        ...
        foreach ($payments as $key => $payment) {
            ...
            $payment->balance = ($lastBalance + $payment->bill + $payment->cash_received) - ($payment->paid + $payment->returned + ($payment->sequence == 'a' ? 0 : $payment->discount));
        }
    ...
    public function addSupplierPayment()
    {
        ...
        try {
            ...
            unset($payment['SPayment_amount']);
            unset($payment['SPayment_discount']);
            $payment['SPayment_amount']   = $this->mt->convert($paymentObj->SPayment_amount);
            $payment['SPayment_discount'] = $this->mt->convert($paymentObj->SPayment_discount);
