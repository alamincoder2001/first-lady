...
class Model_Table extends CI_Model
{
    ...
    {
        $transferInvoice = "T00001";

        $lastTransferInvoice = $this->db->query("select * from tbl_transfermaster order by transfer_id desc limit 1");
        if ($lastTransferInvoice->num_rows() != 0) {
            $newTransferInvoice = $lastTransferInvoice->row()->transfer_id + 1;
            $zeros = array('0', '00', '000', '0000');
            $transferInvoice = 'T' . (strlen($newTransferInvoice) > count($zeros) ? $newTransferInvoice : $zeros[count($zeros) - strlen($newTransferInvoice)] . $newTransferInvoice);
        }
    ...
    public function getBankTransactionSummary($accountId = null, $date = null)
    {
        $bankTransactionSummary = $this->db->query("
            select 
                ...
                (
                    select ifnull(sum(sm.SaleMaster_PaidAmount), 0) from tbl_salesmaster sm
                    where sm.account_id = ba.account_id
                    and sm.Status = 'a'
                    and sm.SaleMaster_branchid = " . $this->session->userdata('BRANCHid') . "
                    " . ($date == null ? "" : " and sm.SaleMaster_SaleDate < '$date'") . "
                ) as total_sales_received_from_customer,
                (
                    select ifnull(sum(pm.PurchaseMaster_PaidAmount), 0) from tbl_purchasemaster pm
                    where pm.account_id = ba.account_id
                    and pm.status = 'a'
                    and pm.payment_type = 'bank'
                    and pm.PurchaseMaster_BranchID = " . $this->session->userdata('BRANCHid') . "
                    " . ($date == null ? "" : " and pm.PurchaseMaster_OrderDate < '$date'") . "
                ) as total_payment_to_supplier,
                (
                    select (ba.initial_balance + total_deposit + total_received_from_customer + total_received_from_supplier + total_sales_received_from_customer) - (total_withdraw + total_paid_to_customer + total_paid_to_supplier + total_payment_to_supplier)
                ) as balance
    ...
    public function supplierDue($clauses = "", $date = null)
    {
        ...
        $supplierDues = $this->db->query("
            ...
            (select ifnull(sum(sp.SPayment_amount + sp.SPayment_discount), 0.00) from tbl_supplier_payment sp 
    ...
    public function customerDue($clauses = "", $date = null)
    {
        ...
        $dueResult = $this->db->query("
            ...
            (select ifnull(sum(cp.CPayment_amount + cp.CPayment_discount), 0.00) 
            ...
            from tbl_customer c
            where c.status = 'a'
            and c.Customer_brunchid = '$branchId' $clauses
        ")->result();
    ...
    public function getSupplierDueById($Suppid)
    {
        ...
        where pm.status = 'a' and pm.Supplier_SlNo = ?", $Suppid)->row();
        $dueAm = $purchaseMaster->dueAmount;

        // ====================
        $paidAmount = $this->db->where('SPayment_customerID', $Suppid)->where('SPayment_TransactionType', 'CR')->where('SPayment_status', 'a')->select_sum('SPayment_amount')->get('tbl_supplier_payment')->row();
        $paidAm = $paidAmount->SPayment_amount;

        // ====================
        $payAmount = $this->db->where('SPayment_customerID', $Suppid)->where('SPayment_TransactionType', 'CP')->where('SPayment_status', 'a')->select_sum('SPayment_amount')->get('tbl_supplier_payment')->row();
        $payAm = $payAmount->SPayment_amount;

        // ====================
        $returnAmount = $this->db->where('Supplier_IDdNo', $Suppid)->where('Status', 'a')->select_sum('PurchaseReturn_ReturnAmount')->get('tbl_purchasereturn')->row();
        ...
        // $due = ($paidAm + $dueAm + $prevDue) - $payAm;
