					...
					<div class="col-md-12">
						<form class="form-horizontal" @submit.prevent="saveBranch">
							...
							<div class="form-group">
								...
								<label class="col-sm-1 control-label no-padding-right">:</label>
								<div class="col-sm-8">
									<input @change="previewImage" type="file" class="form-control" style="height:35px;" />
								</div>
							</div>
						...
						<table class="table table-bordered">
							...
							<tbody>
								<tr v-for="(branch, sl) in branches">
									<td>{{ sl + 1 }}</td>
									<td>  
										<img :src="'/uploads/Branch/'+branch.branch_logo" alt="" width="120px" height="auto">
									</td>
...
<script>
    new Vue({
        ...
        methods: {
            ...
            saveBranch(){
				let fd = new FormData();
				fd.append('branch_logo', this.selectedFile);
                ...
                axios.post(url, fd, this.branch).then(res => {
                    let r = res.data;
                    alert(r.message);
                    if(r.success){
                        this.getBranches();
