	...
	new Vue({
		...
		methods: {
			...
			calculateNetPayable(employee) {
				let payable = ((parseFloat(employee.salary) + parseFloat(employee.benefit / 100)) - parseFloat(employee.deduction / 100)).toFixed(2);
			...
			async getEmployees() {
				...
				await axios.post('/check_payment_month', {
						month_id
					})
					.then(res => {
						this.payment = false;
