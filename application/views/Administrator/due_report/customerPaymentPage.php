				...
				<div class="row">
					...
					<div class="col-md-5">
						...
						<div class="form-group">
							...
							<label class="col-md-1">:</label>
							<div class="col-md-7">
								<input type="number" step="0.01" class="form-control" v-model="payment.CPayment_amount" required>
							</div>
						</div>
						<div class="form-group">
							...
							<label class="col-md-1">:</label>
							<div class="col-md-7">
								<input type="number" step="0.01" class="form-control" v-model="payment.CPayment_discount" required>
							</div>
						</div>
						<div class="form-group">
				...
				<datatable :columns="columns" :data="payments" :filter-by="filter" style="margin-bottom: 5px;">
					<template scope="{ row }">
						<tr>
							...
							<td>{{ moment(row.CPayment_date).format("DD/MM/YYYY") }}</td>
	...
	new Vue({
		...
		methods: {
			...
			saveCustomerPayment() {
				...
				let conf = confirm('Are you sure to save payment?');
				if (conf) {
			...
			editPayment(payment) {
				...
				this.payment.CPayment_amount = this.payment.CPayment_amount * 100;
				this.payment.CPayment_discount = this.payment.CPayment_discount * 100;
			},
			...
			resetForm() {
				...
				this.payment.CPayment_amount = 0;
