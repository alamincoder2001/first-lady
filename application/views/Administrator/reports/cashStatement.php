...
<div id="cashStatement">
    <div class="row" style="border-bottom: 1px solid #ccc;">
        <div class="col-md-12">
            <form action="" class="form-inline" @submit.prevent="getStatements();dateSet()">
        ...
        <div class="row" style="padding-top:10px;">
            <div class="col-md-6">
                ...
                <table class="table table-bordered table-condensed">
                    ...
                    <tbody style="display:none;" v-bind:style="{display: salesBank.length > 0 ? '' : 'none'}">
                        <tr v-for="sale in salesBank">
                            <td>{{ sale.SaleMaster_InvoiceNo }}</td>
                            <td>{{ sale.SaleMaster_SaleDate }}</td>
                            <td>{{ sale.Customer_Name }}</td>
                            <td style="text-align:right;">{{ sale.SaleMaster_PaidAmount | decimal }}</td>
                    ...
                    <tfoot>
                        <tr style="font-weight:bold;">
                            ...
                            <td style="text-align:right;">
                                <span v-if="sales.length == 0">0.00</span>
                                <span style="display:none;"
                                    v-bind:style="{display: salesBank.length > 0 ? '' : 'none'}">{{ totalSalesBank | decimal }}</span>
                            </td>
                        </tr>
                    </tfoot>
                </table>
...
new Vue({
    ...
    computed: {
        ...
        totalSalesBank() {
            return this.salesBank.reduce((prev, curr) => {
                return prev + parseFloat(curr.SaleMaster_PaidAmount)
            }, 0).toFixed(2);
        },
        totalPurchase() {
        ...
        totalCashIn() {
            return parseFloat(this.totalSales) +
                parseFloat(this.totalSalesBank) +
    ...
    methods: {
        ...
        getSales() {
            axios.post('/get_sales', this.filter)
                .then(res => {
                    this.sales = res.data.sales.filter((obj) => {
                        return obj.payment_type != 'bank'
                    });

                    this.salesBank = res.data.sales.filter((obj) => {
                        return obj.payment_type != 'cash'
                    });
                })
        },
        ...
        getSales_2() {
            ...
            axios.post('/get_sales', this.filter2)
                .then(res => {
                    this.sales_2 = res.data.sales.filter((obj) => {
                        return obj.payment_type != 'bank'
                    });
                })
        },
